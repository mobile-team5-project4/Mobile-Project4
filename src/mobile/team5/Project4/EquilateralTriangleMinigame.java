...
public class EquilateralTriangleMinigame extends Minigame {
	...
	EquilateralTriangleMinigame(Context con, int width, int height) {
		...
		x = (int)(c60 * (fixedPoint1.x - fixedPoint2.x) - s60 * (fixedPoint1.y - fixedPoint2.y) + fixedPoint2.x);
		y = (int)(s60 * (fixedPoint1.x - fixedPoint2.x) + c60 * (fixedPoint1.y - fixedPoint2.y) + fixedPoint2.y);
		...
		x = (int)(cm60 * (fixedPoint1.x - fixedPoint2.x) - sm60 * (fixedPoint1.y - fixedPoint2.y) + fixedPoint2.x);
		y = (int)(sm60 * (fixedPoint1.x - fixedPoint2.x) + cm60 * (fixedPoint1.y - fixedPoint2.y) + fixedPoint2.y);
	...
	public void gameDraw(Canvas c) {
		...
		c.drawLine(fixedPoint1.x, fixedPoint1.y, fixedPoint2.x, fixedPoint2.y, paint);

		//if (pointSet)
		paint.setColor(Color.BLUE);
		c.drawLine(fixedPoint1.x, fixedPoint1.y, 
					user.x, user.y, paint);
		c.drawLine(fixedPoint2.x, fixedPoint2.y, 
				user.x, user.y, paint);

		if (submitted) {
			paint.setColor(Color.GREEN);
			if(getDistance(finalPoint,user) < getDistance(mirrorFinalPoint,user))
			{
	...
	public Double getScore() {
		...
		double score = Math.ceil(difference1+difference2);
